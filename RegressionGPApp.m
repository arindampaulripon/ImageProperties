classdef RegressionGPApp < handle
    %
    % Usage:
    %     app.RegressionGPApp
    ...
    properties (Constant)
        AppPathBase = {'/RegressionGP'};
        AppClass = 'RegressionGPApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            RegressionGPApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'predictFunction')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong predictFunction
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'predictFunction').getString, ...
            ...
            if nargout(@predictFunction) == 0
                eval('predictFunction');
            else
                obj.AppHandle = eval('predictFunction');
            end
